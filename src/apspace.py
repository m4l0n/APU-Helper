...
class APSpace:
    ...
    async def login(self, credentials: dict) -> str:
        ...
        if ticket_html.status == 201:
            ticket_text = await ticket_html.text()
            soup = BeautifulSoup(ticket_text, "lxml")
...
async def main():
    ...
    await apspace_session.login({'username': 'TP062253', 'password': '2TRY!vK6JTCF'})
    try:
...
if __name__ == "__main__":
    # API_KEY = 'da2-dv5bqitepbd2pmbmwt7keykfg4'
